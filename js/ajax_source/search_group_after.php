...
if($total < 1) {
    ...
    if(
    (
    (is_array($_GET['search_type_'.$_SESSION['user']['secure_spam_key']]) 
    ...
    else
    {
        ...
        if(count($sub_sports) > 1)
        {
            ...
            {
                //$output.="Sportart: ".$sub_sport['sport_group_name'].":".$sub_sport['ANZAHL']."<br>";

                //$sub_query_sport_sql= // JOINS ?
                $sub_query_sport_sql=" AND utsgv.sport_group_id = '".$sub_sport['sport_group_id']."' ";
                //$total_subquery=get_total_subquery($base_search_sql.$sub_query_type_sql.$sub_query_sport_sql);

                //echo $base_search_sql.$sub_query_type_sql.$sub_query_sport_sql."<br><br>";
                ...
                $output.='
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-12">
                            <a href="'.SITE_URL.'search/?search_single_sport_group_'.$_SESSION['user']['secure_spam_key'].'='.$sub_sport['sport_group_id'].$search_url_terms_string.$search_url_types_string.'" class="thumbnail thumbnail-line">
                            <div class="row">
            ...
            $output.='<div class="row" id="showMoreDiv'.$_SESSION['search_load_more'].'">
                    ...
                    function showMoreGroups'.$_SESSION['search_load_more'].'(page)
                        {
                        $("#moreGroupsContent'.$_SESSION['search_load_more'].			'").load("'.SITE_URL.'js/ajax_source/search_group_after.php", '.json_encode($_GET).');
