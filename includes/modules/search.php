...
if(isset($_GET['search_action_'.$_SESSION['user']['secure_spam_key']]) && $_GET['search_action_'.$_SESSION['user']['secure_spam_key']] == "search.do") {
	// Wenn Radius gesetzt
		...
		$search_sql.=" AND (acos(sin(RADIANS(geo.city_lat))*sin(RADIANS(".$search_lat."))+cos(RADIANS(geo.city_lat))*cos(RADIANS(".$search_lat."))*cos(RADIANS(geo.city_lng) - RADIANS(".$search_lng.")))*".RADIUS.") < ".$_GET['search_radius_'.$_SESSION['user']['secure_spam_key']];
...
//&& $_GET['page'] == 0
            ...
            foreach ($_GET['search_type_'.$_SESSION['user']['secure_spam_key']] as $search_type) {
                ...
                if($total_subquery > 0) {
                    ...
                    if($_SESSION['logged_in'] == 0) {
                    $output .= '<a data-toggle="modal"   data-target="#register_modal" class="thumbnail thumbnail-line">';
                        } 
                    else {
                        $output .= '<a href="#SITE_URL#search/?search_type_'.$_SESSION['user']['secure_spam_key'].'[]='.$search_type.$search_url_terms_string.'" class="thumbnail thumbnail-line">';
                    }
