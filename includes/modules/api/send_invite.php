...
if(friendship_exists($used_in_profile_id, $friendship_user['user_id']) === true) {
    header(HEADER_SERVERERR);
    $response['code'] = ALREADY_REGISTERED;
    die(json_encode($response));
}

$sql = "INSERT INTO `user_friendship` (`user_id`, `friendship_user_id`, `friendship_confirmed`, `friendship_date`) VALUES ('$used_in_profile_id', '$invite_user_id', 0, NOW());";
$query = $DB->prepare($sql);
$query->execute();
...
build_history_log($used_in_profile_id,"friendship_requeset", $friendship_user['user_id']);
