...
$sql = "DELETE FROM `user_friendship` WHERE friendship_user_id='$invite_user_id' AND user_id = '$used_in_profile_id'";
$query = $DB->prepare($sql);
$query->execute();

if($query->rowCount() == 1) {
    build_history_log($used_in_profile_id,"friendship_pre_deleted", $invite_user_id);
}